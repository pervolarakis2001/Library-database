...
IF (new.school_users_id = (SELECT bor.school_users_id from borrowings bor INNER JOIN school_users s ON s.school_users_id = bor.school_users_id INNER JOIN books b ON  b.ISBN = bor.ISBN
    WHERE s.status = "student" AND bor.ISBN = new.ISBN  and s.school_users_id=new.school_users_id  GROUP BY bor.school_users_id HAVING COUNT(*)= 2) AND DATEDIFF(new.borrowing_date,CURDATE()) <7 ) THEN 
      SIGNAL SQLSTATE '45000'
           SET MESSAGE_TEXT = 'check constraint on  borrowings failed - A student can only borrow 2 books a week.';
    END IF;

IF (new.school_users_id = (SELECT bor.school_users_id from borrowings bor INNER JOIN school_users s ON s.school_users_id = bor.school_users_id INNER JOIN books b ON  b.ISBN = bor.ISBN
    WHERE s.status = "teacher" AND bor.ISBN = new.ISBN and s.school_users_id=new.school_users_id  GROUP BY bor.school_users_id HAVING COUNT(*)= 1) AND DATEDIFF(new.borrowing_date,CURDATE()) <7 ) THEN 
      SIGNAL SQLSTATE '45000'
